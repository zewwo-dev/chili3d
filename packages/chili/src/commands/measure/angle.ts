...
export class AngleMeasure extends MultistepCommand {
    ...
    private readonly getThirdPointData = (): PointSnapData => {
        return {
            ...
            validator: (point: XYZ) => {
                return (
                    this.stepData[0].point!.distanceTo(point) > Precision.Distance &&
                    this.stepData[1].point!.distanceTo(point) > Precision.Distance
                );
            },
    ...
    private readonly arcPreview = (point: XYZ | undefined) => {
        ...
        const line2 = this.meshLine(this.stepData[0].point!, point, VisualConfig.highlightEdgeColor, 3);
        const arc = this.application.shapeFactory
            .arc(
                normal,
                this.stepData[0].point!,
                this.stepData[0].point!.add(v1.multiply(this.lineLength(point) * ARC_POSITION)),
                angle,
            )
    ...
    private lineLength(point: XYZ | undefined) {
        const d1 = this.stepData[0].point!.distanceTo(this.stepData[1].point!);
        if (!point) {
            return d1;
        }
        const d2 = this.stepData[0].point!.distanceTo(point);
        return Math.min(d1, d2);
    }

    private arcInfo(point: XYZ) {
        const v1 = this.stepData[1].point!.sub(this.stepData[0].point!).normalize()!;
        const v2 = point.sub(this.stepData[0].point!).normalize()!;
    ...
    protected override executeMainTask(): void {
        const { v1, rad, normal } = this.arcInfo(this.stepData[2].point!);
        const arcMid = v1
            .rotate(normal, rad * 0.5)!
            .multiply(this.lineLength(this.stepData[2].point) * ARC_POSITION)
            .add(this.stepData[0].point!);
        const visualId = this.document.visual.context.displayMesh([
            this.meshPoint(this.stepData[2].point!),
            ...this.arcPreview(this.stepData[2].point),
        ]);
