...
import { command, Matrix4, type XYZ } from "chili-core";
...
export class Move extends TransformedCommand {
    ...
    private readonly movePreview = (point: XYZ | undefined) => {
        ...
        if (!point) return [p1];
        return [p1, this.transformPreview(point), this.getTempLineData(this.stepData[0].point!, point)];
    };

    protected override transform(point: XYZ): Matrix4 {
        const { x, y, z } = point.sub(this.stepData[0].point!);
