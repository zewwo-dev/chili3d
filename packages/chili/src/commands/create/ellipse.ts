...
import { EllipseNode } from "../../bodies/ellipse";
...
export class Ellipse extends CreateFaceableCommand {
    ...
    private readonly getRadius1Data = (): SnapLengthAtPlaneData => {
        ...
        return {
            ...
            plane: (tmp: XYZ | undefined) => this.findPlane(this.stepData[0].view, point, tmp),
    ...
    private readonly validatePoint = (point: XYZ) => {
        ...
        const plane = this.findPlane(this.stepData[0].view, center, point);
    ...
    protected previewCircle = (end: XYZ | undefined) => {
        ...
        const plane = this.findPlane(this.stepData[0].view, this.stepData[0].point!, end);
        return [
            ...
            this.meshCreatedShape(
                ...
                end.distanceTo(this.stepData[0].point!),
            ),
        ];
    };
    ...
    private readonly getRadius2Data = (): LengthAtAxisSnapData => {
        ...
        const direction = plane.normal.cross(this.stepData[1].point!.sub(point)).normalize()!;
        return {
            point: point,
    ...
    protected override geometryNode(): GeometryNode {
        const [p0, p1, p2] = [this.stepData[0].point!, this.stepData[1].point!, this.stepData[2].point!];
        const plane = this.stepData[1].plane!;
    ...
    private readonly ellipsePreview = (point: XYZ | undefined) => {
        if (!point) return this.previewCircle(this.stepData[1].point);
    ...
    private createEllipse(p2: XYZ) {
        const p0 = this.stepData[0].point!;
        const p1 = this.stepData[1].point!;
        const plane = this.stepData[1].plane!;
